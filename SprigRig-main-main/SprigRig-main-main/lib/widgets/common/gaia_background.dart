...
class GaiaBackground extends StatelessWidget {
  final Widget child;
  final Color primaryColor;

  const GaiaBackground({
    super.key, 
    required this.child,
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        ...
        // Subtle organic overlay (Gaia's Gaze)
        Positioned.fill(
          child: CustomPaint(
            painter: _GaiaPainter(primaryColor),
          ),
        ),
...
class _GaiaPainter extends CustomPainter {
  final Color color;
  _GaiaPainter(this.color);
